	...
	public function work_choose($work_id, $to_status, $trust_response = false) {
		...
		if ($to_status == 4) { 
			if($this->_task_status==7){
				...
				$feed_arr = array ("feed_username" => array ("content" => $work_info ['uid'], "url" => "index.php?do=space&member_id={$work_info ['uid']}" ), "action" => array ("content" => $_lang ['success_bid_haved'], "url" => "" ), "event" => array ("content" => $this->_task_title, "url" => "index.php?do=task&task_id=$this->_task_id", 'cash' => $cash_info ['real_cash'] ) );
